  ...
  <el-row :gutter="20" type="flex">
    <el-col :span="12">
      <j-card>
        ...
        <ve-histogram v-on-echart :extend="lineBarOption.extend" :series="lineBarOption.series" :yAxis="lineBarOption.extend.yAxis" :colors="colors"></ve-histogram>
      </j-card>
    </el-col>
    <el-col :span="12">
      <j-card>
...
<script>
import { getWholeData, getWholeTable } from '../api/api.js'
...
export default {
  ...
  methods: {
    ...
    getLegendData (val) {
      ...
      this.lineBarOption.extend.legend.data[0] = `${data}(${unit})`
      this.lineBarOption.extend.legend.data[1] = '同比增长（%）'
      this.lineBarOption.extend.series[0].name = `${data}(${unit})`
      this.lineBarOption.extend.series[1].name = '同比增长（%）'
      this.lineBarOption.extend.yAxis[0].name = unit
    ...
    getWholeData () {
      ...
      getWholeData(data).then((res) => {
        ...
        if (data) {
          ...
          this.lineBarOption.extend.series[0].data = salesData;
          this.lineBarOption.extend.series[1].data = contrastData;
          // 零售额Y设置
          const max = Math.ceil(Math.max.apply(
            Math,
            salesData.map((o) => {
              return o;
            })
          ))
          const min = Math.ceil(Math.min.apply(
            Math,
            salesData.map((o) => {
              return o;
            })
          ))
          this.lineBarOption.extend['yAxis.0.min'] = min;
          this.lineBarOption.extend['yAxis.0.max'] = max;
          this.lineBarOption.extend['yAxis.0.interval'] = Math.ceil((max - min) / 5)
          // 同比Y设置
          const max1 = Math.ceil(Math.max.apply(
            Math,
            contrastData.map((o) => {
              return o;
            })
          ))
          const min1 = Math.ceil(Math.min.apply(
            Math,
            contrastData.map((o) => {
              return o;
            })
          ))
          this.lineBarOption.extend['yAxis.1.min'] = min1-1;
          this.lineBarOption.extend['yAxis.1.max'] = max1;
          this.lineBarOption.extend['yAxis.1.interval'] = Math.ceil((max1 - min1) / 5)
        }
      });
    },
    getWholeTable () {
